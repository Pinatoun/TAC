...
public class MCD {
	...
	// T(n) = 6+4+2+3+5+7n+5n+15n^2+(7/2)n^3-5-15n-(7/2)n^2+10+1+1+5+3+3+3+1+1+1 = 44-3n+(23/2)n^2+(7/2)n^3
	public static BigInteger descomposicion(BigInteger n1, BigInteger n2) {
		...
		while (!n1.equals(one) || !n2.equals(one)) { //5+7n+(n-1)(1+1+3+15n+(7/2)n^2)+2+2+1+2+2+1 (peor caso, ambos son el mismo numero primo)
	...
	// Metodo que determina cual es el siguiente numero primo al proporcionado |T(n) = 2+n(2+12+7n/2+1)+1 = 3+15n+(7/2)n^2
	public static BigInteger siguientePrimo(BigInteger n){
		...
		}while(!isPrimo2(n)); // n(2+12+7n/2+1)
		return n; //1
	}
	...
	public static boolean isPrimo1(BigInteger n) {
		...
		for (BigInteger i = new BigInteger("2"); n.divide(two).compareTo(i) > 0; i = i.add(one)) { //2+3+n(2+3+2)
	...
	// Metodo que determina si un numero es primo con pasos de 2 | T(n) = 4+2+2+3+(n/2)(2+3+2)+1 = 12+7n/2
	public static boolean isPrimo2(BigInteger n) {
		...
		for (BigInteger i = new BigInteger("3"); n.divide(two).compareTo(i) > 0; i = i.add(two)) { //2+3+(n/2)(2+3+2)
			if (n.mod(i).equals(zero)) {
				return false;
			}
		}
